...
// Компонент Hero — это главная навигационная панель (хедер) интернет-аптеки. 

// Страница отвечает за:
// верхнюю информационную строку (телефон и кнопка «Версия для слабовидящих»),
// основное навигационное меню с кнопкой «Главная» (логотип), 
// выпадающим каталогом (Popover + Menu с категориями лекарственных препаратов и статичными разделами) и интеркактивной кнопкой «Аптеки»,
// строку поиска с динамическими подсказками. 
// В зависимости от текущего маршрута поиск работает либо по товарам (на главной, в каталоге), либо по адресам аптек (на странице карты аптек).

// Пользовательскую секцию: если пользователь авторизован, показывает имя, ссылку в личный кабинет и кнопку выхода; 
// если гость — ссылки на регистрацию и вход.

//Кнопку корзины со счётчиком товаров.

// Сам Hero адаптивный: на экранах шириной менее 1024px рендерится мобильная версия HeroMobile (отдельный компонент, получающий все необходимые пропсы собранными в объект mobileProps).
// На десктопах используется развёрнутая структура с Popover и Menu из Headless UI.
