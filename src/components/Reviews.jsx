...
// Компонент Reviews — это статическая секция «Отзывы покупателей». Он демонстрирует социальное доказательство и повышает доверие к интернет-аптеке.
// Блок содержит заголовок, подзаголовок и три карточки с отзывами, каждая из которых включает:
// Эмодзи-аватар в зелёном круге,имя, дату отзыва, рейтинг в виде пяти звёздочек (жёлтые — заполненные, серые — пустые), текст отзыва.
// Данные отзывов хранятся в локальной константе reviews и не загружаются с сервера — это типичная заглушка, используемая в учебных и демонстрационных проектах. 
// Компонент полностью статичен, не принимает пропсов и не выполняет побочных эффектов. 
// Адаптивная вёрстка обеспечивает отображение в одну колонку на мобильных устройствах и в три колонки на средних и больших экранах.