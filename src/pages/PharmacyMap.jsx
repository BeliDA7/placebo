...
// Компонент PharmacyMap представляет собой страницу с интерактивной картой аптек. Он включает:
// Карту на основе библиотеки pigeon-maps с отображением маркеров всех аптек из статического массива pharmacies.
// Пользователь может двигать карту и масштабировать её.
// Карточку активной аптеки, которая появляется при клике на маркер. 
// Содержит название, адрес, телефон, время работы, кнопку избранного и кнопку построения маршрута через Яндекс.Карты.
// Список всех аптек под картой с возможностью быстрого перехода к нужной.
// Плавающую панель «Избранное» (только для авторизованных пользователей), которая показывает сохранённые аптеки в виде кликабельных кнопок.
// Поиск аптек: через пропсы searchQuery и searchTrigger можно искать аптеку по адресу или названию — карта центрируется на найденной,
// и показывается карточка. Если аптека не найдена — выводится уведомление.
// Работу с избранным: добавление и удаление через favoritesApi, синхронизация с сервером при авторизации.
// Компонент экспортирует массив pharmacies как pharmaciesList для использования в других модулях (Hero, Account). 
// Это один из ключевых функциональных компонентов приложения.
// Из-за сложностей с настройкой карты был создан дубликат PharmacyMapSelect.jsx для страницы выбора аптеки при оформлении заказа.
