...
// Компонент ProductPage реализует страницу детального просмотра отдельного товара. 
// Он отвечает за: Получение товара по id из URL (через useParams), поиск в статическом массиве allProducts.
// 
// Отображение полной информации о товаре:
// цветной плейсхолдер вместо изображения,
// название, описание, цена (текущая и старая при скидке),
// статус наличия с цветным бейджем,
// предупреждение для рецептурных препаратов,
// кнопка добавления в корзину (неактивна, если товара нет в наличии),
// характеристики: страна-производитель, форма выпуска, условия хранения.

// Навигацию: кнопка «Назад» (возвращает на предыдущую страницу через navigate(-1)), а при отсутствии товара — сообщение и кнопка возврата на главную.
// 
// Добавление в корзину через колбэк onAddToBasket, переданный от родительского компонента (без всплывающего уведомления).
// Компонент является презентационным и полностью полагается на данные из статического массива allProducts.
// При отсутствии товара с указанным id выводит сообщение об ошибке, что делает страницу устойчивой к некорректным URL.